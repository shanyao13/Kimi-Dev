...
class NVEmbedCode(BaseEmbedding):
    def __init__(
        self,
        model_path: str,
        task_instruction: str = "general",
        **kwargs,
    ):
        ...
        self._query_prefix = f"Instruct: {task_instructions[task_instruction]}\nQuery: "
    ...
    def _get_query_embedding(self, query: str) -> list[float]:
        return self._model.encode(
            [query], prompt=self._query_prefix, normalize_embeddings=True
        )[0].tolist()
...
class EmbeddingIndex(ABC):
    def __init__(
        ...
        **kwargs,
        ...
        self.embedding_model_path = kwargs.get("embedding_model_path", None)
    ...
    def retrieve(self, mock=False):
        ...
        if not os.path.exists(persist_dir) or mock:
            ...
            elif self.embedding_model == "NV-Embed-Code":
                embed_model = NVEmbedCode(model_path=self.embedding_model_path)
            else:
